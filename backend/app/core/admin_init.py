...
from app.core.permissions import DefaultRoles
import logging
...
def create_super_admin_user(db: Session, tenant: Tenant, org: Organization, dept: Department) -> User:
    """创建超级管理员用户"""
    # 检查是否已存在超级管理员
    ...
    if existing_username:
        logger.warning(f"用户名 {settings.SUPER_ADMIN_USERNAME} 已存在，使用邮箱作为用户名")
...
def assign_super_admin_role(db: Session, user: User, tenant: Tenant):
    ...
    super_admin_role = db.query(Role).filter(
        ...
        Role.name == DefaultRoles.SUPER_ADMIN
    ).first()
