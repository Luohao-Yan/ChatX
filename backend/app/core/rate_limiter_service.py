...
class RateLimiterService:
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            if custom_limit:
                ...
                limit_config = self.default_rate_limits.get(action, {"requests": 10, "window": 60})
    ...
    async def record_failed_login(
        ...
        try:
            ...
            logger.warning(f"记录失败登录: {identifier}, IP: {ip_address}, 账户失败: {account_failures}, IP失败: {ip_failures}")
    ...
    async def check_login_attempts(
        ...
        try:
            ...
            if account_locked or ip_blocked:
                # 获取重置时间
                account_ttl = await redis_client.ttl(account_key) if account_locked else 0
                ip_ttl = await redis_client.ttl(self._get_login_attempts_key(f"ip:{ip_address}")) if ip_blocked and ip_address else 0
    ...
    async def clear_login_attempts(self, identifier: str, ip_address: str = None) -> bool:
        """清除登录尝试记录（成功登录后调用）"""
        try:
            redis_client = await get_redis()
    ...
    async def add_to_ip_blacklist(self, ip_address: str, duration_seconds: int) -> bool:
        ...
        try:
            ...
            success = await redis_client.setex(blacklist_key, duration_seconds, blacklist_data)
            
            if success:
                logger.warning(f"IP {ip_address} 已添加到黑名单，持续时间: {duration_seconds}秒")
    ...
    async def get_rate_limit_status(self, identifier: str, action: str) -> Dict[str, Any]:
        """获取限流状态"""
        try:
            redis_client = await get_redis()
            rate_limit_key = self._get_rate_limit_key(identifier, action)
