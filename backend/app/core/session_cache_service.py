...
class SessionCacheService:
    ...
    async def cache_session(self, session: UserSession) -> bool:
        ...
        try:
            ...
            if session.expires_at:
                expire_seconds = int((session.expires_at - datetime.now(timezone.utc)).total_seconds())
                expire_seconds = max(60, min(expire_seconds, self.session_expire_seconds))  # 最少1分钟，最多7天
            else:
            ...
            await redis_client.setex(
                f"{user_sessions_key}:{session.id}", 
                expire_seconds, 
                session.id
            )
            ...
            logger.debug(f"会话 {session.id} 已缓存到Redis，过期时间: {expire_seconds}秒")
            return True
        ...
        try:
            ...
            if session_data:
                ...
                if session_data.get("expires_at"):
                    ...
                    expire_seconds = max(60, min(expire_seconds, self.session_expire_seconds))
                else:
                    expire_seconds = self.session_expire_seconds
    ...
    async def deactivate_all_user_sessions(self, user_id: int) -> bool:
        ...
        try:
            ...
            cursor = 0
            while True:
                cursor, keys = await redis_client.redis_client.scan(cursor, match=pattern, count=100)
