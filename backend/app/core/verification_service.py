    ...
    async def create_verification_code(
        ...
        try:
            ...
            if await redis_client.exists(cooldown_key):
                ...
                logger.warning(f"用户 {user_id} 验证码创建过于频繁，剩余冷却时间: {remaining_time}秒")
                return None
            ...
            if success:
                ...
                logger.info(f"为用户 {user_id} 创建了 {verification_type} 验证码，过期时间: {expire_time}秒")
    ...
    async def verify_code(
        ...
        try:
            ...
            if stored_code == input_code:
                ...
                await redis_client.setex(attempts_key, self.default_expire_seconds, new_attempts)
                
                remaining_attempts = self.max_attempts - new_attempts
                logger.warning(f"用户 {user_id} 验证码验证失败，剩余尝试次数: {remaining_attempts}")
