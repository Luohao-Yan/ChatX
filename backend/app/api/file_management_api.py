...
from app.schemas.file_schemas import (
    ...
    FolderCreate, FolderUpdate, Folder as FolderSchema, FolderTree,
    FileSearchParams, FileUploadResponse, MultiFileUploadResponse,
    FileShare as FileShareSchema, FileShareCreate, FileShareUpdate,
    FileComment as FileCommentSchema, FileCommentCreate, FileCommentUpdate,
    FileStatistics, UserFileStatistics
)
...
async def get_file_detail(
    ...
    # 记录活动
    file_service._log_file_activity(
        file_id, current_user.id, "view"
    )
...
async def update_file(
    ...
    # 记录活动
    file_service._log_file_activity(
        file_id, current_user.id, "update", update_data
    )
...
async def download_file(
    ...
    try:
        ...
        # 记录活动
        file_service._log_file_activity(
            file_id, current_user.id, "download"
        )
...
async def get_user_statistics(
    ...
    recent_activity = db.query(FileActivity).filter(
        FileActivity.user_id == current_user.id
    ).order_by(FileActivity.created_at.desc()).limit(10).all()
