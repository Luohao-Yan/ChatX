...
from app.schemas.recycle_bin_schemas import (
    ...
    BatchRestoreRequest, BatchPermanentDeleteRequest,
    OperationResult, RecycleBinStats
)
...
async def get_recycle_bin_items(
    ...
    if not resource_type or resource_type == "user":
        deleted_users = await user_service.get_users_list(
            current_user, skip=(page-1)*per_page, limit=per_page, include_deleted=True
        )
        # 过滤出已删除的用户
    ...
    if not resource_type or resource_type == "role":
        deleted_roles = await role_service.get_tenant_roles(current_user, include_deleted=True)
    ...
    for item in restore_request.items:
        try:
            if item.resource_type == "user":
                ...
                await user_service.user_repo.update(item.resource_id, {
                    "is_deleted": False,
                    "deleted_at": None,
                    "deleted_by": None,
                    "is_active": True
                ...
                # 恢复角色
                await role_service.role_repo.update(item.resource_id, {
                    "is_deleted": False,
                    "deleted_at": None,
                    "deleted_by": None
                })
