...
from app.shared.multi_tenancy.tenant import tenant_context
...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if (
            ...
            and not self._is_file_upload(request)
        ):
        ...
        try:
            ...
            if (
                ...
                and self._should_log_response_body(response)
            ):
                response_body = await self._get_response_body(response)

            # 设置完整的请求上下文
            if tenant_id:
                set_request_context(request_id=request_id, tenant_id=str(tenant_id))
            ...
            log_performance_event(
                ...
                duration_ms,
                {"status_code": response.status_code, "client_ip": client_ip},
            )
    ...
    def _should_log_response_body(self, response: Response) -> bool:
        ...
        if any(
            ct in content_type
            for ct in ["image/", "video/", "audio/", "application/octet-stream"]
        ):
            return False
