  ...
  return (
    <div className='space-y-8'>
      ...
      <div className='space-y-2'>
        <label className='text-sm font-medium'>{t('settings.appearance.font')}</label>
      ...
      <div className='space-y-1'>
        ...
        <div className='flex max-w-4xl gap-8 pt-2'>
          {/* Light Theme */}
          <div>
            <button
              type='button'
              ...
              className={cn(
                'border-muted hover:border-accent items-center rounded-md border-2 p-1 w-full cursor-pointer flex flex-col',
          ...
          {colorSchemes.map((scheme) => (
            <button
              ...
              className={cn(
                ...
                colorScheme === scheme.value ? 'border-primary' : 'border-muted-foreground/20'
              )}
            >
          ...
          {radiusOptions.map((option) => (
            <button
              ...
              className={cn(
                ...
                'border-muted-foreground/20 hover:bg-accent hover:text-accent-foreground hover:border-accent',
                !useCustomRadius && radius === option.value ? 'bg-primary/10 border-primary text-primary' : 'bg-background'
              )}
            >
              <div
                className={cn(
                  'w-8 h-8 mb-2 transition-colors',
                  !useCustomRadius && radius === option.value ? 'bg-primary' : 'bg-muted-foreground/60'
                )}
          ...
          {(['xs', 'sm', 'md', 'lg', 'xl'] as const).map((size) => (
            <button
              ...
              className={cn(
                ...
                'border-muted-foreground/20 hover:bg-accent hover:text-accent-foreground hover:border-accent',
                fontSize === size ? 'bg-primary/10 border-primary text-primary' : 'bg-background'
              )}
            >
              <div className={cn(
                'mb-2 font-medium',
      ...
      <div className='space-y-6'>
        ...
        <div className='grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3'>
          {(['none', 'fade', 'slide', 'slide-up', 'zoom', 'blur-fade'] as const).map((transition) => (
            <button
              key={transition}
              type='button'
              ...
              className={cn(
                'flex flex-col items-center p-4 rounded-md border-2 transition-colors',
                'border-muted-foreground/20 hover:bg-accent hover:text-accent-foreground hover:border-accent',
                pageTransition === transition ? 'bg-primary/10 border-primary text-primary' : 'bg-background'
              )}
            >
              ...
              <span className='text-xs text-center'>
                {transition === 'none' ? '无动画' :
                 ...
                 transition === 'slide-up' ? '垂直滑动' :
                 transition === 'zoom' ? '缩放' :
                 transition === 'blur-fade' ? '模糊渐变' : transition}
              </span>
            </button>
          ))}
        </div>
