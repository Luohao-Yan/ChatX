...
import { DepartmentList, DepartmentForm } from '../components/department'
import { useDepartments, useOrganizations } from '../hooks'
...
export default function DepartmentsPage() {
  ...
  const {
    ...
    deleteDepartment,
    refreshData,
  } = useDepartments({ organizationId: selectedOrganizationId })

  const { organizations } = useOrganizations()

  const breadcrumbItems = [
    { label: t('nav.knowledge') },
    { label: t('nav.departmentLib') }
  ]
  ...
  const handleDelete = async (department: Department) => {
    if (confirm(t('department.deleteConfirm'))) {
      try {
